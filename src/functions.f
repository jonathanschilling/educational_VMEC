...
!> \brief Profile function for the two_power profile.
!>        \f$b(0) * (1 - x^{b(1)})^{b(2)}\f$
!>
...
!> \brief Profile function for the two_power_gs profile.
!>        \f$\texttt{two_power}(x)*(1 + Sum[b(i)*Exp(-(x - b(i+1))/b(i+2))^2])\f$
!>
!> @param x evaluation location
!> @param b parameter vector
