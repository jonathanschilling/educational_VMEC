...
SUBROUTINE freeb_data (rmnc, zmns, rmns, zmnc, bmodmn, bmodmn1)
  ...
  DO iprint = 1, 2
     ...
     DO l = 1, nzeta, nzskip
        ...
        IF (iprint .eq. 1) THEN
           DO k = 1, ntheta2
              lk = l + nzeta*(k - 1)
              WRITE (nthreed, 770) zeta, rb(lk), zb(lk), (bsqsav(lk,n),n=1,3), bsqvac(lk)
           END DO
        ELSE
           DO k = 1, ntheta2
              lk = l + nzeta*(k - 1)
  ...
  IF (lasym) THEN
     ...
     WRITE (nthreed, 910) NINT(xn(mn)/nfp), NINT(xm(mn)), rmnc(mn),     &
        zmns(mn), rmns(mn), zmnc(mn), potsin, potcos,                   &
  ...
  ELSE
     ...
     DO mn = 1, mnmax
        ...
        WRITE (nthreed, 810) NINT(xn(mn)/nfp), NINT(xm(mn)),            &
            rmnc(mn), zmns(mn), potsin, bmodmn(mn), bmodmn1(mn)
     END DO
  END IF
...
900 FORMAT(//,3x,'nb',2x,'mb',6x,'rbc',9x,'zbs',9x,'rbs',9x,'zbc',      &
              6x,'vacpot_s',4x,'vacpot_c',2x,'|B|_c(s=.5)',             &
              1x,'|B|_c(s=1.)'/)
